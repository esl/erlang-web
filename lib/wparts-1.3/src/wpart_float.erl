...
%% Ltd. Portions created by Erlang Training & Consulting Ltd are Copyright 2008, 2009,
...
get_html_tag(Attrs, Default, Precision) when is_float(Default) ->
    case Precision of
        undefined ->
            Value = float_to_list(Default);
        N when is_integer(N) ->
            Value = lists:flatten(io_lib:format("~.*..f", [N, Default]))
    end,
    get_html_tag(Attrs, Value, none);
get_html_tag(Attrs, Default, _Precision) ->
    wpart_gen:build_html(wpart_gen:tpl_get(float),
			 [{"html", wpart:proplist2html([{"value", Default} | Attrs])}]).

load_tpl() ->
    wpart_gen:load_tpl(float,
		       filename:join([code:priv_dir(wparts),"html","float.tpl"])).
