...
%% @todo fetch type of the collection element from the params
...
get_html_tag(Name, _Default, Type, Extra) ->
    [{_, Parts}] = ets:lookup(templates, {wpart, collection}),
    HTML = wpart_gen:build_html(Parts, [{"name", Name},
					{"extra", Extra},
					{"type", Type}]),
    
    {XML, _} = xmerl_scan:string(HTML),
    element(2, regexp:gsub(lists:flatten(wpart_xs:process_xml(XML)), "\n", "")).
